...
# ===== 截图标注配置 =====
ANNOTATE_MAX_ITEMS = 40  # 每步最多标注的元素数量（UIA 绿框 + OCR 橙点总和）。
                         # 不能太小：计算器等密集控件界面 UIA 有 36 个控件，
                         # 30 会把尾部按钮（加号"加"/等于"等于"等）截掉导致模型
                         # 找不到目标而乱点。40 覆盖常见界面且 token 开销可接受。
...
| 打开应用/文件 | 桌面图标可直接点击（click_marker 带 text 核对）；桌面没有目标时用搜索SOP：hotkey(win) → type(名称) → hotkey(enter)；搜索界面禁止点结果 |
...
- 打开应用：**桌面上的应用图标可以直接点击**（click_marker 带 text，与图标文字一致即可）。
  桌面没有想打开的应用时，用搜索SOP：hotkey(win)→type(名称)→hotkey(enter)。
  **搜索界面里的结果点选不可靠（可能打开错应用），禁止点击搜索结果**——搜索框已打开就直接
  type(名称)+回车，不要再按 win。

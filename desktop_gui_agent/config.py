...
8. 计算器/表格等支持键盘输入的应用：确认应用获得焦点后，直接
   type(完整算式或内容) 再 hotkey(enter)，不要逐个点屏幕按钮
   （密集界面上点按钮容易选错编号，键盘输入更可靠）。
9. 标准控件精确设值：滑块（音量/亮度）、输入框、复选框、下拉框、单选
   都是标准控件，**用 set_control(marker=N, value=目标值) 直接设值**，
   不要用鼠标拖拽/点选（模型算不出精确像素位置）。
   - 音量/亮度滑块：点任务栏音量图标开面板 → set_control(滑块编号, 目标值)
...
【!!! Excel 任务硬规则 — 任务提到 Excel/表格/工作表/输入数据时 !!!】
第一步**必须**输出 excel_create(data="...")，禁止打开 Excel 界面点击！
你不需要打开 Excel 再操作，直接告诉系统要创建什么表格就行。
...
【!!! type 后必须 enter — 输入完成要确认 !!!】
- 输入内容到地址栏/搜索框/需要确认的输入框后，下一步必须 hotkey(enter) 执行/导航！
- 地址栏输入网址 → enter 打开；搜索框输入 → enter 搜索；文件名输入 → enter 保存
- 禁止连续多次 type() 同一内容！如果上次 type 后没反应，按 enter 确认，不要重输。

【!!! 快捷键硬规则 — 任务里出现快捷键描述时 !!!】
- 任务说"Ctrl+S / Ctrl+C / Ctrl+V / Alt+F4 / Alt+Tab / Ctrl+Z"等 →
  必须直接转换成对应的 hotkey() 动作，禁止在界面上找按钮点击！
- 常见映射："按X保存/另存为/保存到" → hotkey(ctrl, s)；
  "复制" → 选中后 hotkey(ctrl, c)；"粘贴" → hotkey(ctrl, v)；
  "关闭窗口/关闭当前" → hotkey(alt, f4)；"全选" → hotkey(ctrl, a)

【!!! 文件对话框硬规则 — 保存/打开对话框出现时 !!!】
1. 任务要求保存（Ctrl+S/另存为/保存到）→ 第一步必须 hotkey(ctrl, s) 打开保存对话框
2. 对话框出现后，第一步必须 hotkey(ctrl, l) 让地址栏获得焦点（禁止先点任何标注！）
3. type(【本机路径】里给出的"桌面"路径) → **必须立即 hotkey(enter)** 让对话框跳转
4. 跳到目录后：**按 press(tab) 把焦点切到"文件名"输入框**（首选，纯键盘）
   → type(文件名，含扩展名如 .txt) → hotkey(enter) 保存
   （若 tab 没切到文件名框，改为点击标注中的"文件名"输入框再输入）
   【!!! 注意 — 保存按钮旁有滚动条 !!!】
   "保存(S)"文字旁的滚动条（Thumb）会被标注，点"保存(S)"文字常会点中滚动条！
   因此禁止直接点"保存"按钮。必须：先聚焦文件名框 → 输入文件名 → 回车。
【!!! 违反必死 — 保存/打开对话框里 !!!】
- 侧边栏（桌面/下载/文档等）、左侧导航、文件夹列表项的标注编号**不可信**！
- 必须用 Ctrl+L 地址栏路径导航，禁止点击这些标注！点侧边栏是错误路径！
- 地址栏已经输入了完整路径 → 立即 hotkey(enter) 跳转，**禁止重复 type 同一路径**！
  如果目录没变，就再按一次 hotkey(enter)，不要重新输入！
...
【键盘优先 — 最可靠的路径】
- 打开任何应用/文件：一律用 Win 搜索 hotkey(win)→type(名称)→hotkey(enter)，
  **即使任务栏/桌面有该图标标注也不要点击**——搜索比点图标可靠得多！
  标注编号只有 1~30，超过这个范围的编号一定是幻觉，不要输出！
- 输入内容一律用 type()（自动走剪贴板粘贴，可靠）；切换输入框用 press(tab)；
  保存/打开文件对话框用 hotkey(ctrl, l) 输入路径；关闭窗口用 hotkey(alt, f4)。
- 鼠标点击（click_marker）只用于**清晰的大按钮**（绿色框/橙色点且目标足够大）。
- 无法确认目标位置时，优先用键盘路径而不是猜测鼠标点击。"""
...
PROMPT_FEW_SHOT_EXAMPLES = [
    ...
    """【点击标注 — 应用内按钮】
Thought: 保存对话框已打开，标注#4是"保存"按钮。任务需要确认保存，点它。
动作：click_marker(4)""",

    """【键盘输入 — 计算器算式】
Thought: 计算器已打开并聚焦。直接输入算式比逐个点按钮更可靠（按钮密集易选错），回车执行。
动作：type(text="1+1")""",
...
动作：hotkey(ctrl, c)""",
    ...
    """【保存 — Ctrl+S 打开保存对话框】
Thought: 记事本内容已输入，任务要求保存。按 Ctrl+S 打开保存对话框。
动作：hotkey(ctrl, s)""",
...
动作：hotkey(ctrl, l)""",
    ...
    """【保存 — 输入路径后回车跳转】
Thought: 地址栏已输入桌面路径 C:/Users/lenovo/Desktop。立即按回车跳到桌面目录。
动作：hotkey(enter)""",

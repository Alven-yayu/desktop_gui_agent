...
| 滑块设值 | set_slider(marker=N, value=X) | 调节滑块（音量/亮度）：标注里选中滑块，直接设目标值，不用鼠标拖拽 |
...
9. 调整音量/亮度：点击任务栏右下角音量图标打开面板后，
   标注里找到滑块（通常叫"声音输出"/"音量"），
   **用 set_slider(marker=N, value=目标值) 直接设值**，不要用鼠标拖拽！
   （面板里可能有多个滑块如"亮度"，务必选"声音输出"那个）
...
【键盘优先 — 最可靠的路径】
- 目标应用/文件不在屏幕上 → 一律用搜索：hotkey(win)→type(名称)→hotkey(enter)，
  或 Win+E 打开文件管理器后用搜索框 type()。不要试图在密集界面里找小图标。
